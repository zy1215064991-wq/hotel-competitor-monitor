...
```text
高德候选池 -> FlyAI/飞猪价格 -> 百度口碑补充 -> WorkBuddy 内置模型分析 -> 按配置推送
```
...
- 口碑补充：百度地图。
- 百度省额度策略：默认启用本地缓存 `data/cache/baidu/`，并用 `data/usage/` 账本按北京时间自然日累计真实调用次数。
- 分析模型：WorkBuddy 内置模型。
- 推送：由 `pushMode` 三选一，支持微信助理 ClawBot、企业微信群机器人或只保存本地；Automation 官方可确认的通知入口是“推送到 WorkBuddy 小程序”。
...
5. 填写本店、城市、商圈/POI、半径、入住口径、竞对数量、品牌加权关键词、百度补充数量。品牌关键词只参与候选评分，不会额外发起一轮品牌搜索。百度额度紧张时，直接点“百度省额度模式”，先跑高德 + FlyAI + 分析 + 推送主链路。
...
9. WorkBuddy 读取 `templates/daily-prompt.md` 和 `data/api-combo/api-combo-latest-report-input.md`，生成日报，再严格按 `pushMode` 执行一种推送路径。首次使用 ClawBot 必须以微信端实际收到消息为验证结果。
...
第二条命令会先备份本地配置，再补齐 `baidu.usageDirectory` 等新增字段，不覆盖已有酒店和查询口径。第三条命令只做零额度验收。
...
- `query.nights`：入住晚数
- `query.roomType`：目标房型
- `query.rooms`、`query.adults`、`query.children`：保留在经营口径和历史指纹中；当前 FlyAI CLI 不接收这些参数，报告会标记 `not-applied-by-flyai-cli`
- `discovery.competitorCount`：最终监控并进入日报/历史的竞对数量
- `discovery.radiusMeters`：高德周边搜索半径
- `discovery.maxCandidates`：高德先拉取的候选池上限，后续会再按价格、可比性评分、排序和竞对数量收口
- `discovery.brandKeywords`：品牌加权关键词，只影响候选可比性评分，不额外扩充候选池
- `discovery.maxPrice`：最高价格筛选，只过滤明确高于上限的候选；价格缺失或脱敏时保留给日报说明
- `discovery.sort`：最终候选排序方式，默认 `balanced` 可比性优先；也支持距离、低价、评分和默认排序。FlyAI 查询会自动映射到飞猪支持的排序值。
...
- `baidu.usageDirectory`：百度自然日额度账本目录，默认 `data/usage`
- `baidu.dailyCallLimit`：百度每天的真实 HTTP 调用上限；同一天多次运行共享账本累计，DryRun 不写账本、不消耗真实额度
...
scripts/scan-secrets.ps1     多类型敏感信息扫描，不回显密钥正文
...
- 如果价格被脱敏，例如 `¥1xx`，只能当作价格带信号，不能计算精确差额、比例或涨跌幅。

## 许可证

本项目使用 MIT License。你可以按许可证条款复制、修改和二次分发，但仍需自行遵守高德、FlyAI/飞猪、百度地图、WorkBuddy 和微信/企业微信等平台的服务条款。
