...
创建酒店竞对每日监控 Automation 时，先开启官方“推送到 WorkBuddy 小程序”。如果当前桌面端实际界面另外提供微信助理 ClawBot 通知入口，再选择最终回复或完整任务结果。

ClawBot 绑定完成不等于 Automation 一定能直接选它。首次试跑必须检查微信端是否实际收到日报；没有实收时标记“ClawBot 尚未验证”，不要只凭任务完成判断成功。
